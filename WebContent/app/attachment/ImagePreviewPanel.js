 ...
 */
Ext.define('app.attachment.ImagePreviewPanel', {
	...
	tbar : [{
				...
				listeners : {
					click : function(button) {
						if (button.pressed) {
							...
							if (p.down('#stretchpanel image').src != p.url)
								p.down('#stretchpanel image').setSrc(p.url);
				...
				listeners : {
					click : function(button) {
						if (button.pressed) {
							var p = button.up('imagepreviewpanel');
							if (p.down('#normalpanel image').src != p.url)
								p.down('#normalpanel image').setSrc(p.url);
				...
				handler : function(button) {
					button.up('imagepreviewpanel').printImage(button
							.up('imagepreviewpanel').url);
				}
			}],
	items : [{
		xtype : 'panel',
	...
	setImage : function(url, width, height) {
		...
		if (this.down('#stretch').pressed)
			this.down('#stretchpanel image').setSrc(url);
		else
			this.down('#normalpanel image').setSrc(url);
	...
	adjustSize : function() {
		...
		if (this.down('#stretch').pressed) {
			...
			if (panelHeight / panelWidth > this.imageHeight / this.imageWidth) {
				...
				imagePanel
						.setPosition(this.imagepading, (panelHeight - factHeight) / 2);
