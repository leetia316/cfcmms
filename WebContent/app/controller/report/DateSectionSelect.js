 ...
 * 附件的控制器
 */
Ext.define('app.controller.report.DateSectionSelect', {
	extend : 'Ext.app.Controller',

	requires : ['app.report.widget.YearMonthSelectMenu'],
		...
		this.control({
			...
			'yearmonthselectmenu datemenu' : {
				select : function(datepicker, date) {
					var menu = datepicker.up('yearmonthselectmenu');
					...
					target.fireEvent('dateSectionChanged', target, {
							...
							//如果是 dateSelectButton 则， menu.dateField 为空，如果是从 navigateSelectedfieldsTree中传过来的，则
							//要去执行 mainreport 的 dataSectionchanged 事件，而 dataField 会作为参数传进去。表示不是基准模块的的定义。
				}
			},
			...
			'yearmonthselectmenu form button' : {
				click : function(button) {
							...
							if (firstyearfield.disabled || lastyearfield.disabled) {
								...
								if (firstyear == lastyear) {
									if (firstmonth == lastmonth) {
										...
										text = firstyear + '年' + firstmonth + '--' + lastmonth
												+ '月';
										result.value = firstyear + '-' + firstmonth + '--'
												+ lastyear + '-' + lastmonth;
									}
								} else {
									text = firstyear + '年' + firstmonth + '月--' + lastyear + '年'
											+ lastmonth + '月';
									result.value = firstyear + '-' + firstmonth + '--' + lastyear
											+ '-' + lastmonth;
								}
							}
							break;
						case 'quartersection' :
							firstquarter = form.getForm().findField('firstquarter')
									.getValue();
							...
							if (firstyearfield.disabled || lastyearfield.disabled) {
								...
								if (firstyear == lastyear) {
									if (firstquarter == lastquarter) {
										...
										text = firstyear + '年' + firstquarter + '--' + lastquarter
												+ '季';
										result.value = firstyear + '-' + firstquarter + '--'
												+ lastyear + '-' + lastquarter;
									}
								} else {
									text = firstyear + '年' + firstquarter + '季--' + lastyear
											+ '年' + lastquarter + '季';
									result.value = firstyear + '-' + firstquarter + '--'
											+ lastyear + '-' + lastquarter;
								}
					...
					result.text = text;
					var menu = form.ownerCt.up('yearmonthselectmenu');
					var target = menu.target;
					if (target && target.hideMenu)
						target.hideMenu();
					else
						menu.close();
					target
							.fireEvent('dateSectionChanged', target, result, menu.dateField);
				}
			},
			...
			'form#monthsection numberfield[name=firstyear]' : {
				change : function(field, value) {
					...
					if (ly.getValue() < value)
						...
						var fm = field.up('form').down('numberfield[name=firstmonth]')
								.getValue();
			...
			'form#quartersection numberfield[name=firstyear]' : {
				change : function(field, value) {
					...
					if (ly.getValue() < value)
						...
						var fm = field.up('form').down('numberfield[name=firstquarter]')
								.getValue();
			...
			'form checkbox[name=_enablefirst]' : {
				change : function(field, value) {
					...
					if (field.nextSibling().nextSibling())
						field.nextSibling().nextSibling()[value ? 'enable' : 'disable']();
					if (!value) {
	...
	dateSelectItemClick : function(e) {
		...
		switch (type) {
			...
			case 'yearmonth' :
				result.value = e.year + '-' + e.month;
				text = e.parentMenu.ownerItem.year + '年' + e.month + '月';
				break;
			case 'yearquarter' :
				result.value = e.year + '-' + e.quarter;
				text = e.parentMenu.ownerItem.year + '年' + e.quarter + '季度';
				break;
			default :
		}
		result.text = text;
		var menu = e.up('yearmonthselectmenu');
		var target = menu.target;
		target.fireEvent('dateSectionChanged', target, result, menu.dateField);
